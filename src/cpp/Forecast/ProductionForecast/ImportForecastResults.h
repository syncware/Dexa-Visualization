...
ForecastResult ImportForecastResults::SetRow(vector<string>& forecastColumnsValues)
{
	...
	int i = -1;
	i++;
	forecastResult.uniqueId = forecastColumnsValues[i]; i++;
	forecastResult.Version_Name = forecastColumnsValues[i]; i++;
	forecastResult.Field = forecastColumnsValues[i]; i++;
	forecastResult.Reservoir = forecastColumnsValues[i]; i++;
	forecastResult.Drainage_Point = forecastColumnsValues[i]; i++;
	forecastResult.Production_String = forecastColumnsValues[i]; i++;
	forecastResult.TRANCHE = forecastColumnsValues[i]; i++;
	forecastResult.Asset_Team = forecastColumnsValues[i]; i++;
	forecastResult.Flow_station = forecastColumnsValues[i]; i++;
	forecastResult.ModuleName = forecastColumnsValues[i]; i++;
	forecastResult.Day = stoi(forecastColumnsValues[i]); i++;
	forecastResult.Month = stoi(forecastColumnsValues[i]); i++;
	forecastResult.Year = stoi(forecastColumnsValues[i]); i++;
	forecastResult.Oil_rate = stod(forecastColumnsValues[i]); i++;
	forecastResult.Gas_Rate = stod(forecastColumnsValues[i]); i++;
	forecastResult.Water_Rate = stod(forecastColumnsValues[i]); i++;
	forecastResult.Liquid_Rate = stod(forecastColumnsValues[i]); i++;
	forecastResult.Cum_Oil_Prod = stod(forecastColumnsValues[i]); i++;
	forecastResult.Cum_Gas_Prod = stod(forecastColumnsValues[i]); i++;
	forecastResult.Cum_Water_Prod = stod(forecastColumnsValues[i]); i++;
	forecastResult.GOR = stod(forecastColumnsValues[i]); i++;
	forecastResult.BSW = stod(forecastColumnsValues[i]); i++;
	forecastResult.CGR = stod(forecastColumnsValues[i]); i++;
	forecastResult.WGR = stod(forecastColumnsValues[i]); i++;
	forecastResult.CutBack = stod(forecastColumnsValues[i]); i++;
	forecastResult.HyrocarbonStream = forecastColumnsValues[i]; i++;
	forecastResult.hydrocarbonType = forecastColumnsValues[i]; i++;
	forecastResult.terminal = forecastColumnsValues[i]; i++;
	forecastResult.URo = stod(forecastColumnsValues[i]); i++;
	forecastResult.URg = stod(forecastColumnsValues[i]); i++;
	forecastResult.Gas_Own_Use = stod(forecastColumnsValues[i]); i++;
	forecastResult.Gas_Demand = stod(forecastColumnsValues[i]); i++;
	forecastResult.Gas_Flared = stod(forecastColumnsValues[i]); i++;
	forecastResult.Crude_Oil_Lossess = stod(forecastColumnsValues[i]); i++;
	forecastResult.CutBack = stod(forecastColumnsValues[i]); i++;
	forecastResult.projectCode = forecastColumnsValues[i]; i++;
	forecastResult.projectName = forecastColumnsValues[i]; i++;
	i++;
	i++;
	i++;
	i++;
	forecastResult.resourceClass = forecastColumnsValues[i]; i++;
	forecastResult.DeclineRate = stod(forecastColumnsValues[i]); i++;
