...
vector<vector<FacilityWellsIndicies>>& facilityWellsIndicies, vector<Date>& dateTimes)
{
	...
	string filePath = FolderName + "/forecast_results_cpp/group_1";
	//========================================================================//
	//Create group_1 folder
	mkdirretval=mkpath(filePath);
	//==========================================================================//
...
string& wellName, string& facilityName)
{
	...
	if(nDates > 0 ){
		...
		string FolderName = "./files";
		string filePath = FolderName + "/forecast_results_cpp/group_1";
		string filePath2 = filePath + "/" + facilityName;
		string filePath3 = filePath2 + "/" + wellName + ".json";
