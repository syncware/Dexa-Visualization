...
string& hyrocarbonStream)
{
    ...
    if(hyrocarbonStream == "oil")
    {
        ...
        {
            if(CurrentCumProduction >= wellActivityResult.externalForecastProfile[i-1].Cum_Oil_Prod
            && CurrentCumProduction <= wellActivityResult.externalForecastProfile[i].Cum_Oil_Prod)
            {
    ...
    else
    {
        for(i = wellActivityResult.startRowIndex + 1; i < nSize; i++)
        {
            if(CurrentCumProduction >= wellActivityResult.externalForecastProfile[i-1].Cum_Gas_Prod
            && CurrentCumProduction <= wellActivityResult.externalForecastProfile[i].Cum_Gas_Prod)
            {
                cumProductionIndex = i - 1;
                return cumProductionIndex;
            }
